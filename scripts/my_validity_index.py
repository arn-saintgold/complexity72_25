...
def process_cluster(cluster_id, X, labels, metric, d, mst_raw_dist, verbose, **kwd_args):
    if np.sum(labels == cluster_id) == 0:
        return None

    distances_for_mst, core_distances = distances_between_points(
        X, labels, cluster_id, metric, d,
        no_coredist=mst_raw_dist,
        print_max_raw_to_coredist_ratio=verbose,
        **kwd_args
    )

    mst_nodes, mst_edges = internal_minimum_spanning_tree(distances_for_mst)
    density_sparseness = mst_edges.T[2].max()

    return cluster_id, core_distances, mst_nodes, mst_edges, density_sparseness
...
def validity_index(X, labels, metric='euclidean',
    ...
    **kwd_args :
    ...
    # Inside validity_index:
    results = Parallel(n_jobs=-1)(
        delayed(process_cluster)(cluster_id, X, labels, metric, d, mst_raw_dist, verbose, **kwd_args)
    ...
    for res in results:
        if res is None:
            continue
        cid, cored, nodes, edges, sparseness = res
        core_distances[cid] = cored
        mst_nodes[cid] = nodes
        mst_edges[cid] = edges
        density_sparseness[cid] = sparseness
